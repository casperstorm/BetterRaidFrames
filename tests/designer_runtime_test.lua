...
assert(square.filters.includeSpellIDs[364343] and square.filters.isFromPlayerOrPlayerPet)
assert(icon.filters.isFromPlayerOrPlayerPet == nil)
...
assert(container.groups[2].filters.includeSpellIDs[774])
assert(not Glow(container.groups[2].frames[1]).shown, "reordering cannot transfer glow to another spell")
assert(container.groups[2].frames[1].point[5] == 22)

local allocationCount = #env.widgets
Addon:RemoveDesignerIndicator("default", c)
Addon:UpdateDesignerIndicators(frame)
assert(next(container.groups[3].filters.includeSpellIDs) == nil)
...
Addon:UpdateDesignerIndicators(frame)
assert(not container.groups[3].frames[1].bindings.Icon and not container.groups[3].frames[1].bindings.DurationCooldown)
assert(container.groups[3].frames[1].bindings.DurationText)
